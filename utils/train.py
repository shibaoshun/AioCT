...
from utils import loss_singleTF
...
parser.add_argument("--lamuda", type=float, default=0.001, help="Tight Frame constraint")
...
def train_model(net, optimizer, lr_scheduler, datasets):
    ...
    for epoch in range(opt.resume, opt.niter):
        ...
        for ii, data in enumerate(data_loader[phase]):
            Xma, GT, XLI, mask, Sgt = [x.cuda() for x in data]
            net.train()

            optimizer.zero_grad()
            X0, ListX, ListA, ListX_nonK, ListA_nonK, W1, S = net(Xma, XLI, mask)
            loss_l2Xt = 0
            loss_l1Xt = 0
            loss_l1At = 0
            newAgt = mask*(Xma - GT)
            ...
            for j in range(opt.T):
                loss_l2Xt = float(loss_l2Xt) +  0.1 * F.mse_loss(ListX[j]*mask, newXgt)#第一项
                loss_l1Xt = float(loss_l1Xt) +  0.1 * torch.sum(torch.abs(ListX[j]*mask - newXgt))#第二项10次求和
                loss_l1At = float(loss_l1At) +  0.1 * torch.sum(torch.abs(mask *ListA[j]-newAgt))#第三项10项求和
            ...
            lossW = criterion(ListX[-1]*mask, newXgt, W1, lamuda)
            Lsino = F.mse_loss(S, Sgt)
            loss = opt.Xl2 * loss_l2X + opt.Xl1* loss_l1X + opt.Al1 * loss_l1A + lossW + Lsino
            # back propagation
            loss.backward()
            optimizer.step()
            mse_iter = loss.item()
            ...
            if ii % 100 == 0:
                ...
                print(template.format(epoch + 1, opt.niter, phase, ii, num_iter_epoch[phase], mse_iter, loss_l2X, loss_l1X, loss_l1A, lossW, Lsino, lr))
...
if __name__ == '__main__':
    ...
    criterion = loss_singleTF.my_single_TF_sum_squared_error()
    criterion.cuda()  # 转化到GPU
